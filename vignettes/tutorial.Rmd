...
plot(cbind(RR_fit$rates[-1], inla_fit$summary.fitted.values$mode[(n_node + 1):(n_node * 2 - 1)]))
```
...
plot(cbind(RR_fit$rates[-1], inla_fit2$summary.fitted.values$mode[(n_node + 1):(n_node * 2 - 1)]))
abline(0, 1)

inla_fit2 <- phinla(phy = tree, data = x, obs_error = "est", hyper = RR_fit$lambda,
                   verbose = FALSE)
summary(inla_fit2)
...
plot(cbind(RR_fit$rates[-1], inla_fit3$summary.fitted.values$mode[(n_node + 1):(n_node * 2 - 1)]))
```
```{r aces_1}
...
inla_fit_o <- phinla(phy = tree, data = x, obs_error = "est", verbose = FALSE)
summary(inla_fit_o)
