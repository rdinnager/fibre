...
test <- pgls(V2 ~ V1, phydat, lambda = "ML", delta = "ML", kappa = "ML")
summary(test)

```

Now let's try setting up the model in `phybr`.
...
res <- phybr(V2 ~ V1, phy, df, phy_match = "tipnames",
             rate_order = "second_order",
             hyper = list(prec = list(prior = "pc.prec", param = c(12, 0.01))))
summary(res)
