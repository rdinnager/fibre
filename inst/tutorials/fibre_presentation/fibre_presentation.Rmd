...
- `{fibre}` currently supports 'Gaussian' (ridge) and 'Laplacian' (LASSO) based models through the `"glmnet"` engine.
...
- Eyeballing caertianly looks like something is happening:
...
```{r latent_birds, exercise=TRUE}
bird_beak_codes
beak_mod <- load_model("bird_beaks")
beak_mod
```

```{r latent_birds-check}
ls()
```
...
```{r bird1, out.height='50%', out.width='50%'}
...
```{r latent_birds2-check}
ls()
```

## Make a mesh from the model

```{r latent_birds3, exercise=TRUE}
mesh <- beak_mod$get_mesh(latent)
shade3d(mesh, col = "gold")
```

```{r latent_birds3-check}
ls()
```
...
- I did this as part of a Postdoc with the I University of Canberra in Australia
...
```{r example1, out.height=700, out.width=1100}
...
```{r example2, out.height=700, out.width=1100}
